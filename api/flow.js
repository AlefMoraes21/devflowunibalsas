...
function calculateEnemAverage(scores) {
  if (!Array.isArray(scores) || scores.length !== 5) {
    throw new Error('São necessárias exatamente cinco notas para calcular a média do ENEM.');
  }

  const numericScores = scores.map((score) => Number(score));

  if (numericScores.some((score) => !Number.isFinite(score))) {
    throw new Error('Não foi possível calcular a média das notas do ENEM.');
  }

  const total = numericScores.reduce((sum, score) => sum + score, 0);
  const average = Math.round(((total / 5) + Number.EPSILON) * 100) / 100;
  ...
  switch (customAction) {
    ...
    case 'enviar_curso': {
      try {
        ...
        const notasEnem = notaEnem
          ? {
              natureza: normalizeEnemScore(
                data.nota_natureza_enem,
                'Nota de ciências da natureza'
              ),
              humanas: normalizeEnemScore(
                data.nota_humanas_enem,
                'Nota de ciências humanas'
              ),
              matematica: normalizeEnemScore(
                data.nota_matematica_enem,
                'Nota de matemática'
              ),
              linguagens: normalizeEnemScore(
                data.nota_linguagens_enem,
                'Nota de linguagens'
              ),
              redacao: normalizeEnemScore(
                data.nota_redacao_enem,
                'Nota da redação'
              )
            }
          : null;

        const mediaEnem = notaEnem
          ? calculateEnemAverage([
              notasEnem.natureza,
              notasEnem.humanas,
              notasEnem.matematica,
              notasEnem.linguagens,
              notasEnem.redacao
            ])
          : '';

        if (notaEnem) {
          console.log('MÉDIA ENEM CALCULADA:', {
            media: mediaEnem
          });
        }
...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    return res.status(200).json({ status: 'ok', version: 'enem-average-auto-v17' });
  }
