...
function calcMediaEnemAuto(data) {
  try {
    const n = (v) => Number(String(v ?? '').replace(',', '.').trim());
    const notas = [
      n(data.nota_natureza_enem),
      n(data.nota_humanas_enem),
      n(data.nota_matematica_enem),
      n(data.nota_linguagens_enem),
      n(data.nota_redacao_enem)
    ];
    if (notas.some(v => !Number.isFinite(v))) return null;
    return Math.round(notas.reduce((a, b) => a + b, 0) / 5);
  } catch {
    return null;
  }
}
...
function normalizeNationality(value) {
  ...
  const isAllowed = NACIONALIDADES_ESTATICAS.some((item) => item.id === nationalityId);
  if (!isAllowed) {
...
function findInput(form, labels = [], fallbackId = null) {
  ...
  const byLabel = inputs.find((input) => normalizedLabels.includes(normalizeText(input.label)));
...
function findButton(form, preferredLabels = ['Avançar', 'Concluir']) {
  ...
  const button = buttons.find((item) => normalized.includes(normalizeText(item.label))) ?? buttons.find((item) => normalizeText(item.label) !== 'voltar');
  if (!button?.button) {
...
function findQueryForField(form, fieldId) {
  return (form?.querys ?? []).find((query) => (query.fields ?? []).some((item) => Number(item.field) === Number(fieldId))) ?? null;
}
...
async function rubeusRequest(path, { method = 'GET', body, timeoutMs = RUBEUS_TIMEOUT_MS } = {}) {
  ...
  console.log('RUBEUS RESPONSE:', { path, status: response.status, duration_ms: Date.now() - startedAt });
  ...
  if (!response.ok || json?.success === false) {
    ...
    if (Array.isArray(json?.errors) && json.errors.length > 0) {
      console.error('RUBEUS ERROR DETAILS:', JSON.stringify(json.errors, null, 2));
    }
  }
  if (!response.ok) {
...
function buildBasicScreenData(formResponse, processId) {
  ...
  return {
    ...
    // FIX: usa 220 para ENEM, 217 para os demais - conforme sua doc
...
async function buildCourseScreenData(formResponse, processId) {
  ...
  const polo = notaEnem ? requireInput(findInput(form, ['CODPOLO'], 38550), 'CODPOLO') : findInput(form, ['CODPOLO'], 38550);
...
function basicRetryData(data, message) {
  return {
    ...
    // FIX: IDPS correto por processo
    idps_valor: toStringValue(data.idps_valor, isNotaEnemProcess(data.processo_seletivo_id) ? '220' : '217'),
...
async function courseRetryData(data, message) {
  ...
  try {
    const technicalFields = [data.campo_area_interesse_id, data.campo_oferta_id, data.campo_polo_id, data.campo_coligada_id, data.campo_filial_id, data.campo_tipo_curso_id].filter(Boolean);
    const requests = [
  ...
  return {
    ...
    // FIX CODPOLO - garante 01 para ENEM
    polo_valor: toStringValue(nextOptions[toStringValue(data.campo_polo_id)]?.value, isNotaEnemContext(data) ? toStringValue(data.polo_valor, '01') : toStringValue(data.polo_valor)),
    ...
    processo_seletivo_id: isNotaEnemContext(data) ? PROCESSO_NOTA_ENEM_ID : toStringValue(data.processo_seletivo_id),
...
async function shiftRetryData(data, message) {
  ...
  try {
    const technicalFields = [data.campo_area_interesse_id, data.campo_oferta_id, data.campo_polo_id, data.campo_coligada_id, data.campo_filial_id, data.campo_tipo_curso_id].filter(Boolean);
    const shiftsResponse = await getDataSource({ datasource: data.datasource, target: data.campo_turno_id, filter: [{ field: toInteger(data.campo_curso_id, 'campo_curso_id'), values: toStringValue(data.curso_id) }], nextFields: technicalFields, token: data.token });
    shifts = flowOptions(extractDataSourceOptions(shiftsResponse));
    nextOptions = shiftsResponse?.data?.nextOptions ?? {};
  } catch (error) {
...
async function almostThereRetryData(data, message) {
  ...
  if (data.datasource_origem) {
    try {
      const originsResponse = await getDataSource({ datasource: data.datasource_origem, target: data.campo_origem_id, filter: [], nextFields: [], token: data.token });
      const dynamicOrigins = flowOptions(extractDataSourceOptions(originsResponse));
      if (dynamicOrigins.length > 0) origens = dynamicOrigins;
    } catch (error) {
  ...
  switch (customAction) {
    ...
    case 'enviar_dados_basicos': {
      try {
        ...
        const fields = compactFields([
          ...
          fieldItem(data.campo_idps_id, toStringValue(data.idps_valor, isNotaEnemProcess(data.processo_seletivo_id) ? '220' : '217')),
    ...
    case 'buscar_turnos': {
      try {
        ...
        return responseForScreen('CURSO_INTERESSE', {
          ...
          polo_valor: toStringValue(nextOptions[toStringValue(data.campo_polo_id)]?.value, isNotaEnemContext(data) ? '01' : ''),
    ...
    case 'enviar_curso': {
      try {
        ...
        console.log('CONTEXTO ENVIAR CURSO:', { processo_seletivo_id: toStringValue(data.processo_seletivo_id), target: toStringValue(data.target), button_id: toStringValue(data.button_id), eh_nota_enem_recebido: toBoolean(data.eh_nota_enem), nota_enem_calculado: notaEnem });
        const campoPoloId = notaEnem ? toStringValue(data.campo_polo_id, '38550') : toStringValue(data.campo_polo_id);
        const poloValor = notaEnem ? toStringValue(data.polo_valor, '01') : toStringValue(data.polo_valor);
        const mediaCalculada = calcMediaEnemAuto(data);
        const mediaFinal = mediaCalculada ?? (data.media_enem ? Number(String(data.media_enem).replace(',', '.')) : null);
        ...
        const fields = compactFields([
          ...
          ...(notaEnem ? [
            ...
            fieldItem(enemFieldId(data.campo_enem_media_id, 38558), mediaFinal !== null ? mediaFinal : normalizeEnemScore(data.media_enem, 'Média das notas do ENEM'))
          ] : []),
    ...
    case 'enviar_informacoes_complementares': {
      try {
        ...
        console.error('Erro ao concluir inscrição:', { message: error.message, applyment_id: toStringValue(data.applyment_id), person_id: toStringValue(data.person_id) });
...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    return res.status(200).json({ status: 'ok', version: 'enem-codpolo-fix-final-v17' });
  }
  if (req.method !== 'POST') {
